...
const LMasterDetailGrids = () => {
  ...
  const expandChange = event => {
    ...
    fetch(baseUrl + `Products?$filter=CategoryID%20eq%20` + categoryID, init).then(response => response.json()).then(json => {
  ...
  React.useEffect(() => {
    fetch(baseUrl + `Categories`, init).then(response => response.json()).then(json => setCategories(json.value));
  }, []);
  return <div>
        <Grid style={{
      height: '550px'
    }} data={categories} detail={DetailComponent} expandField="expanded" onExpandChange={expandChange}>
...
export default LMasterDetailGrids;